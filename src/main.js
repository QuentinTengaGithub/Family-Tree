...
const store = new Vuex.Store({
  ...
  actions: {
    async fetchMembers({ commit, state }, paramUserId = null) {
      const userId =
        state.viewingAs || paramUserId || (state.user ? state.user.uid : null)
      if (!userId) {
        commit('setMembers', [])
        return
      }
      ...
      members = members.map(member => {
        ...
        if (member.siblings) {
          newMember.siblings = member.siblings
            .map(getMemberName)
            .filter(Boolean)
        }
        if (member.children) {
          newMember.children = member.children
            .map(getMemberName)
            .filter(Boolean)
        }
        return newMember
    ...
    async addMember({ commit, state }, member) {
      ...
      const memberData = {
        ...member,
        createdBy: state.viewingAs || state.user.uid,
      }
      await setDoc(
        doc(
          db,
          'users',
          state.viewingAs || state.user.uid,
          'members',
          member.name
        ),
        memberData
    ...
    async deleteMember({ commit, state, dispatch }, memberName) {
      ...
      const membersToUpdate = state.members.filter(
        m =>
          m.married === memberName ||
          m.siblings.includes(memberName) ||
          m.children.includes(memberName)
      )
      ...
      for (const member of membersToUpdate) {
        ...
        if (updatedMember.siblings.includes(memberName)) {
          updatedMember.siblings = updatedMember.siblings.filter(
            s => s !== memberName
          )
        }
        if (updatedMember.children.includes(memberName)) {
          updatedMember.children = updatedMember.children.filter(
            c => c !== memberName
          )
        }
    ...
    async setSiblings({ dispatch, state }, { member, selectedMember }) {
      ...
      const membersToUpdate = state.members.filter(m =>
        fullSiblingGroup.has(m.name)
      )

      for (const memberToUpdate of membersToUpdate) {
        const newSiblings = [...fullSiblingGroup].filter(
          name => name !== memberToUpdate.name
        )
        if (
          JSON.stringify(memberToUpdate.siblings) !==
          JSON.stringify(newSiblings)
        ) {
    ...
    async updateMemberName({ dispatch, state }, { oldName, newName }) {
      ...
      if (oldMemberSnap.exists()) {
        ...
        await setDoc(
          doc(db, 'users', userId, 'members', newName),
          memberData
        )
        ...
        for (const member of allMembers) {
          ...
          if (
            ...
            updatedData.siblings.includes(oldName)
          ) {
            updatedData.siblings = updatedData.siblings.map(s =>
              s === oldName ? newName : s
            )
            needsUpdate = true
          }
          if (
            ...
            updatedData.children.includes(oldName)
          ) {
            updatedData.children = updatedData.children.map(c =>
              c === oldName ? newName : c
            )
            needsUpdate = true
          }
    ...
    async migrateData({ dispatch, state }) {
      ...
      const confirmation = confirm(
        'This will migrate your Firestore data. It will delete and recreate documents to use names as IDs. This is a sensitive operation. Are you sure you want to proceed?'
      )
      if (!confirmation) {
        return
      }
      ...
      for (const member of newAllMembers) {
        ...
        if (memberData.siblings && memberData.siblings.length > 0) {
          ...
          memberData.siblings = memberData.siblings.map(
            s => idMap.get(s) || s
          )
          if (
            JSON.stringify(originalSiblings) !==
            JSON.stringify(memberData.siblings)
          ) {
            needsUpdate = true
          }
        }
        if (memberData.children && memberData.children.length > 0) {
          ...
          if (
            ...
            JSON.stringify(memberData.children)
          ) {
            needsUpdate = true
          }
        }
    ...
    async unsetSibling({ dispatch, state }, { member, siblingName }) {
      ...
      if (member1) {
        const newSiblings = member1.siblings.filter(
          s => s !== siblingName
        )
        dispatch('updateMember', { ...member1, siblings: newSiblings })
      }
      if (member2) {
        const newSiblings = member2.siblings.filter(
          s => s !== member.name
        )
